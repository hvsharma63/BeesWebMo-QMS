...
function pay ( $params, $salt ){
	...
	$payment = new Payment( $salt, PAYU_MODE );
...
class Misc {
	...
	public static function show_page ( $result )
	{
		...
		{
			if ( ! filter_var($result['data'], FILTER_VALIDATE_URL) === false) {
...
class Payment {
	...
	public function pay ( $params = null )
	{
		...
		if ( $error === true ) {
			...
			return array ( 
				'status' => Misc::SUCCESS, 
				'data' => $this->url . ( PAYU_MODE == "prod" ? "_payment" : "_payment_options" ) . '?mihpayid=' . $transaction_id );				
		} else {
